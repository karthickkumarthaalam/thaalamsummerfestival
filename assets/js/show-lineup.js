...
async function fetchLineupTabs() {
  try {
    const response = await fetch(
      `https://api.thaalam.ch/summer-festival/api/showlineup?language=${language}`,
    );
...
function bindTabSwitch() {
  document.querySelectorAll(".tab-titles li").forEach((tab) => {
    tab.addEventListener("click", () => {
      document
        .querySelectorAll(".tab-titles li")
...
async function fetchShowLineupByDate(date, tabNumber) {
  ...
  try {
    const response = await fetch(
      `https://api.thaalam.ch/summer-festival/api/showlineup/by-date?date=${date}`,
    );
...
function renderLineup(data, tabNumber) {
  ...
  const tableBody = document.getElementById(
    `eventDetailsTableBody${tabNumber}`,
  );
  const artistsCarousel = document.getElementById(
    `artistsCarousel${tabNumber}`,
  );
  ...
  data?.data?.shows.forEach((show) => {
    ...
    show.artists.forEach((artist) => {
      ...
      card.innerHTML = `
                <img src="https://api.thaalam.ch/summer-festival/api/${artist.image}" alt="${artist.name}">
