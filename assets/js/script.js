(function ($) {
  ...
  if ($(".course-carousel-style.artist").length) {
    $(".course-carousel-style.artist").owlCarousel({
      loop: true,
      margin: 10,
      nav: false,
      dots: true,
  ...
  $(window).on("load", function () {
    ...
    <div class="cookie-banner__content">
      <div class="cookie-text">
        ...
        <p>To give you the best experience, we use cookies to personalize content, provide social media features, and analyze traffic.</p>
        <p>Essential cookies are always active. You can manage other preferences anytime.</p>
      </div>
      <div class="cookie-banner__buttons">
        <button id="acceptCookies">Accept All</button>
        <button id="moreInfo"><a href="/privacy-policy" target="_blank" style="color: #fff;">More Info</a></button>
      </div>
    </div>
  `;
      ...
      document.getElementById("moreInfo").onclick = (e) => {
        e.stopPropagation();
      };
    }
  });
...
(function () {
  ...
  function loadTranslations() {
    fetch("assets/translation/translation.json")
      .then(res => res.json())
      .then(data => {
        translation = data;
        setupLanguageDropdown();
        const savedLang = localStorage.getItem("selectedLang") || "EN";
        setSelectedLanguage(savedLang, false);
  ...
  function setupLanguageDropdown() {
    document.querySelectorAll(".dropdown-item").forEach(item => {
      item.addEventListener("click", function (e) {
        ...
        setSelectedLanguage(lang, true);
      });
    });
  }

  function setSelectedLanguage(lang, shouldReload = false) {
  ...
  function updateLanguage(lang) {
    const languageKey = translation[lang];
    if (!languageKey) return;

    // Update text content
    document.querySelectorAll("[data-key]").forEach((el) => {
      const key = el.getAttribute("data-key");
      const value = resolveNestedKey(languageKey, key);
      if (value !== undefined) {
        el.innerHTML = value;
      }
    });
